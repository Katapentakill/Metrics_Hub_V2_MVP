...
export default function VolunteerSubmissions() {
  ...
  // Filtrar documentos de tipo "volunteer"
  const documents = mockDocuments.filter(
    (doc) =>
      doc.type === 'volunteer-personal' &&
      doc.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">📝 Volunteer Submissions</h1>
      <p className="text-gray-600 mb-6">
        Revisa todos los documentos cargados por los voluntarios.
      </p>
        ...
        {documents.map((doc) => (
          <Card key={doc.id} className="hover:shadow-lg transition-shadow">
            ...
            <CardContent>
              ...
              <button
                className="mt-2 px-3 py-1 bg-purple-500 text-white rounded-lg hover:bg-purple-600 text-sm flex items-center gap-1"
