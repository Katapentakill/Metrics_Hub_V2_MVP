...
// Badges con colores según guía
const getReasonBadge = (reason: TerminationCertificate['reason']) => {
  switch (reason) {
    case 'completed':
      return <span className="text-xs text-green-700 bg-green-50 px-2 py-1 rounded-full font-medium">
        Program Completed
      </span>;
    case 'resignation':
      return <span className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded-full font-medium">
        Resignation
      </span>;
    case 'relocation':
      return <span className="text-xs text-purple-600 bg-purple-50 px-2 py-1 rounded-full font-medium">
        Relocation
      </span>;
    case 'other':
      return <span className="text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded-full font-medium">
        Other
      </span>;
  }
};
...
const CertificateCard = ({ cert, onToggleFavorite, onIssueCertificate }: {
  ...
  <div className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-[#1e293b] truncate">{cert.volunteerName}</h3>
            <p className="text-xs text-[#4b5563] truncate">{cert.position}</p>
          </div>
        </div>
        <button onClick={() => onToggleFavorite(cert.id)} className="p-1 hover:bg-gray-50 rounded transition-colors">
    ...
    <div className="p-4">
      <div className="grid grid-cols-2 gap-3 mb-3 text-sm">
        <div>
          ...
          <p className="font-medium text-[#1e293b]">{cert.department}</p>
        </div>
        <div>
          <p className="text-xs text-[#4b5563]">Total Hours</p>
          <p className="font-medium text-[#059669]">{cert.totalHours} hrs</p>
        </div>
        <div>
          <p className="text-xs text-[#4b5563]">Service Period</p>
          <p className="font-medium text-[#1e293b]">{calculateServiceDuration(cert.startDate, cert.endDate)}</p>
        </div>
        <div>
          <p className="text-xs text-[#4b5563]">End Date</p>
          <p className="font-medium text-[#1e293b]">{formatDate(cert.endDate)}</p>
        </div>
      </div>

      {cert.achievements && cert.achievements.length > 0 && (
        <div className="mb-3">
          <p className="text-xs text-[#4b5563] mb-1">Achievements</p>
          <div className="flex flex-wrap gap-1">
            {cert.achievements.slice(0, 2).map(achievement => (
              <span key={achievement} className="text-xs bg-green-50 text-[#059669] px-2 py-0.5 rounded-full">
                {achievement}
              </span>
            ))}
            {cert.achievements.length > 2 && (
              <span className="text-xs bg-gray-100 text-[#4b5563] px-2 py-0.5 rounded-full">
                +{cert.achievements.length - 2}
              </span>
            )}
          </div>
      ...
      {cert.notes && (
        <p className="text-sm text-[#4b5563] italic mb-3 line-clamp-2">"{cert.notes}"</p>
      )}

      {cert.certificateIssued && cert.issuedBy && (
        <div className="text-xs text-gray-500 pt-3 border-t border-[#e2e8f0]">
    ...
    <div className="p-4 bg-gray-50 border-t border-[#e2e8f0] flex gap-2">
      {cert.certificateIssued ? (
        <>
          <button className="flex-1 px-3 py-2 bg-gradient-to-r from-[#15803d] to-[#14532d] text-white text-sm rounded-lg hover:shadow-lg transition-all flex items-center justify-center gap-2">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="font-semibold text-[#1e293b]">{cert.volunteerName}</h3>
              {cert.certificateIssued ? (
                <span className="flex items-center gap-1 text-xs text-green-700 bg-green-50 px-2 py-1 rounded-full font-medium">
            ...
            <p className="text-sm text-[#4b5563] mb-2">{cert.position} • {cert.department}</p>
            ...
            {cert.notes && (
              <p className="text-sm text-[#4b5563] italic mb-2">"{cert.notes}"</p>
            )}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={() => onToggleFavorite(cert.id)} className="p-2 hover:bg-gray-50 rounded transition-colors">
              <Star className={`w-5 h-5 ${cert.isFavorite ? 'fill-yellow-400 text-yellow-400' : 'text-gray-400'}`} />
            </button>
            {cert.certificateIssued ? (
              <button className="px-4 py-2 bg-gradient-to-r from-[#15803d] to-[#14532d] text-white text-sm rounded-lg hover:shadow-lg transition-all flex items-center gap-2">
                <Download className="w-4 h-4" />
                Download
              </button>
            ) : (
  ...
  <div className="bg-white rounded-lg border border-slate-200 hover:border-[#059669] hover:shadow-sm transition-all duration-150 p-3">
    <div className="flex items-center justify-between gap-4">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <span className="font-medium text-[#1e293b] text-sm truncate">{cert.volunteerName}</span>
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        {cert.certificateIssued ? (
          <button className="p-2 bg-gradient-to-r from-[#15803d] to-[#14532d] text-white text-sm rounded-lg hover:shadow-lg transition-all">
            <Download className="w-4 h-4" />
          </button>
        ) : (
          <button
...
export default function TerminationCertificatesPage() {
  ...
  const handleIssueCertificate = (id: string) => {
    console.log('Issuing certificate for:', id);
  };
  ...
  const isFilterActive = 
    ...
    filters.certificateStatus !== 'All Status' || 
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-[#1e293b]">Certificados de Terminación</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
            ...
            <StatsCard icon={CheckCircle} label="Issued" value={stats.issued} color="emerald" />
            <StatsCard icon={Clock} label="Pending" value={stats.pending} color="yellow" />
            <StatsCard icon={TrendingUp} label="Total Hours" value={`${stats.totalHours.toLocaleString()}`} color="blue" />
          </div>
        </div>
          ...
          {showFilters && (
            <div className="pt-4 border-t border-[#e2e8f0]">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-[#4b5563] mb-2">Department</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-[#4b5563] mb-2">Reason</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-[#4b5563] mb-2">Certificate Status</label>
                  <select
        ...
        {filteredCertificates.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-xl font-semibold text-[#1e293b] mb-2">No certificates found</h3>
