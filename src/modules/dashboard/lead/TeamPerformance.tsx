...
export default function TeamPerformance({ data }: TeamPerformanceProps) {
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      ...
      case 'down': return <TrendingUp className="w-4 h-4 text-slate-500 rotate-180" />;
  ...
  const getTrendColor = (trend: string) => {
    switch (trend) {
      ...
      case 'down': return 'text-slate-600';
    ...
    <div className="card p-6">
          ...
          <div key={index} className="flex items-center justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center justify-between mt-1 text-xs text-gray-600">
                ...
                <span className={`flex items-center font-medium ${getTrendColor(metric.trend)}`}>
            ...
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-sm font-medium text-slate-700">{member.productivity}%</span>
                </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-200">
        <div className="grid grid-cols-2 gap-4 text-center">
          <div>
            <p className="text-2xl font-bold text-emerald-600">{data?.thisMonthCompletions || 0}</p>
            <p className="text-xs text-gray-600">Tareas Este Mes</p>
          </div>
          <div>
