// src/modules/projects/lead/modals/ProjectModalEdit.tsx
// Modal de edición para Líder de Proyecto - Sin cambio de líder
...
interface Props {
  ...
  leads: ExtendedUserWithProfile[]; // Los líderes no pueden cambiar el líder
...
export default function ProjectModalEdit({ project, leads, onSave, onClose }: Props) {
  const [form, setForm] = useState<Partial<Project>>({
    ...
    lead_id: project?.lead_id || '', // No se puede cambiar
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-30">
      <div className="card w-full max-w-2xl p-6">
        <h3 className="text-lg font-semibold text-slate-800 mb-4">
          {project ? 'Editar Mi Proyecto' : 'Nuevo Proyecto'}
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs text-slate-600">Líder del Proyecto</label>
            <input
              className="input-field w-full bg-gray-100 cursor-not-allowed"
              value={project?.lead_id ? 'No se puede cambiar' : 'Sin asignar'}
              disabled
              title="No tienes permisos para cambiar el líder del proyecto"
            />
          </div>
