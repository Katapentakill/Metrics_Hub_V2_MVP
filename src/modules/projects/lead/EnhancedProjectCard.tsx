// UBICACIÓN: src/modules/projects/lead/EnhancedProjectCard.tsx
// Tarjeta de proyecto para Líder de Proyecto - Sin opción de eliminar
...
interface EnhancedProjectCardProps {
  ...
  onEdit: () => void;
  onDelete: () => void; // Los líderes no pueden eliminar, pero mantenemos la interfaz
  showMenu?: boolean;
  onMenuToggle?: () => void;
  menuId?: string;
}
...
export default function EnhancedProjectCard({ 
  ...
  onEdit, 
  onDelete, // Los líderes no pueden eliminar proyectos
  showMenu = false,
  onMenuToggle,
  menuId 
  ...
  // Cálculos de métricas optimizados con useMemo
  const metrics = useMemo(() => {
    ...
    // Velocidad fija basada en el progreso para evitar cambios constantes
    const velocity = Math.round(progressPct * 0.8 + 10);
  ...
  }, [proj.deadline, proj.current_team_size, proj.max_team_size, progressPct]);

  const { daysUntilDeadline, isUrgent, isOverdue, teamUtilization, velocity } = metrics;
              ...
              {showMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-20">
                  ...
                  <button 
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-gray-50 text-sm border-t border-gray-100 text-gray-400 cursor-not-allowed"
                    disabled
                  >
                    <AlertTriangle className="w-4 h-4 text-gray-400" />
                    <span>Eliminar (No disponible)</span>
                  </button>
                </div>
              )}
            </div>
